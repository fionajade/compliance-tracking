<div class="sidebar glass">

    <h2>Employee Panel</h2>

    <ul>
        <li><a href="__DIR__ . '/../dashboard.php">Dashboard</a></li>


        <li><a href="__DIR__ . '/../activity-logs.php">Activity Logs</a></li>

        <li><a href="__DIR__ . '/../tasks.php">Tasks</a></li>

        <li><a href="__DIR__ . '/../submit-report.php">Submit Report</a></li>

        <li><a href="__DIR__ . '/../compliance.php">Compliance</a></li>

        <li><a href="__DIR__ . '/../my-reports.php">My Reports</a></li>
    </ul>

    <a class="logout" href="__DIR__ . '/../edit-account.php">Edit Account</a>
    <br><br>
    <a class="logout" href="../includes/logout.php">Logout</a>
</div>
